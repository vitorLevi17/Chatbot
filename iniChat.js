...
const model = genAI.getGenerativeModel({ model: "gemini-1.0-pro",tools},
  {apiVersion: "v1beta"}
);
...
function iniChat(){
    chat = model.startChat({
        history: [
          {
            role: "user",
            parts: [{ text: " Você é Cenner, um chatbot amigável que representa a empresa Jornada Viagens, que vende pacotes turísticos para destinos nacionais e internacionais. Você pode responder mensagens que tenham relação com viagens. " }],
          },
          {
            role: "model",
